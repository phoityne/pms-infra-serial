...
module PMS.Infra.Serial.App.ControlSpec (spec) where
...
import qualified PMS.Infra.Serial.App.Control as SUT
import qualified PMS.Infra.Serial.DM.Type as SUT
    ...
    context "when echo command issued." $ do
      it "should call callback" $ \ctx -> do 
        ...
        let domDat = ctx^.domainDataSpecContext
            ...
            cmdQ   = domDat^.DM.serialQueueDomainData
            resQ   = domDat^.DM.responseQueueDomainData
            expect = "abc"
            jsonR  = def {DM._jsonrpcJsonRpcRequest = expect}
            argDat = DM.SerialEchoCommandData jsonR ""
            args   = DM.SerialEchoCommand argDat
